...
- `llm-engineering/` — Building on LLMs: escalate architecture only when needed (call → chain → agent → multi-agent), tool design where the description is the interface, schema-enforced structured outputs, context budgeting and cache-friendly prompt layout, reliability and cost telemetry, evals before prompt-tweaks, treating skills/system prompts as code with the description as the trigger, prompt-injection defense via capability limitation.
...
- `worktree/` — Working with repositories using bare worktrees: wrapper layout with a bare `.git` and sibling worktrees, creating parallel worktrees from `main`/`master`, recursive `.env` file syncing without `node_modules`, context switching, and safe cleanup.
