...
6. If `.devcontainer/` folder exists, generates `.devcontainer/devcontainer.override.json` preserving/detecting the base `image` (from `devcontainer.json` or fallback `"image": "mcr.microsoft.com/devcontainers/javascript-node:24"`) alongside the `.git` mount.
...
4. If `.devcontainer/` folder exists, generates `.devcontainer/devcontainer.override.json` with detected `image` and `.git` mount.
...
- **Devcontainer git visibility & image config**: When `.devcontainer/` is present, `wt-add.sh` (and repo conversion/clone scripts) creates `.devcontainer/devcontainer.override.json` ensuring the base `image` is specified (auto-detected from `devcontainer.json`, or defaulting to `"mcr.microsoft.com/devcontainers/javascript-node:24"`) along with the `source=${localWorkspaceFolder}/../.git` bind mount into `${containerWorkspaceFolder}/../.git`.
