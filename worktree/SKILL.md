...
6. If `.devcontainer/` folder exists, generates `.devcontainer/devcontainer.override.json` preserving and merging all original settings and mounts with `workspaceFolder` set to `/workspaces/${localWorkspaceFolderBasename}`, `workspaceMount`, and the `.git` mount into `/workspaces/.git`.
...
7. Checks if `.devcontainer/` folder or `devcontainer.json` exists in the new worktree; if present, generates `.devcontainer/devcontainer.override.json` preserving and merging all original configuration and existing `mounts` with `workspaceFolder`, `workspaceMount`, and `.git` mount (`source=${localWorkspaceFolder}/../.git,target=/workspaces/.git,type=bind`) so devcontainers can access git repository history and status while keeping all original mounts intact.
...
4. If `.devcontainer/` folder exists, generates `.devcontainer/devcontainer.override.json` merging original configuration and mounts with `workspaceFolder`, `workspaceMount`, and `.git` mount.
...
- **Devcontainer git visibility & workspace mount config**: When `.devcontainer/` is present, `wt-add.sh` (and repo conversion/clone scripts) creates `.devcontainer/devcontainer.override.json` preserving all existing configuration and merging all mounts with `workspaceFolder` set to `/workspaces/${localWorkspaceFolderBasename}`, `workspaceMount` set to `source=${localWorkspaceFolder},target=/workspaces/${localWorkspaceFolderBasename},type=bind,consistency=cached`, and `mounts` including `source=${localWorkspaceFolder}/../.git,target=/workspaces/.git,type=bind` alongside any existing mounts.
